...
Помимо [глобальных атрибутов](/html/doka/global-attrs/) при работе с таблицами вам могут очень пригодиться атрибуты `colspan` и `rowspan`. Оба этих атрибута предназначены для растягивания ячеек. `colspan` для растягивания ячейки на 2 или более столбцов, а `rowspan` для растягивания на 2 или более ряда.

Разделим название телефонов на производителя и модель. Производитель будет повторяться, растянем ячейку с его названием по высоте. Добавим один заголовок «Производитель», а в первые `<tr>` каждого из `<tbody>` и применим атрибут `rowspan`. Теперь эти ячейки занимают собой пространство в 3 и 2 ряда соответственно.
