...
- `download` — если кликнуть по такой ссылке, браузер предложит пользователю скачать то, что по ней находится. Это может быть файл или другая страница — главное, чтобы они находились на том же домене, что и ссылка. Можно задать название скачиваемому файлу, если добавите значение атрибуту. Например, `<a href="/?z=484c08ca" download="фотки.zip">` предложит скачать файл фотки.zip. Пользователь сможет его изменить при скачивании.
...
- `target` — определяет, где откроется ссылка: в том же окне, на новой вкладке или в новом окне браузера. Без этого атрибута содержимое ссылки откроется в той же вкладке. Вот все варианты, где можно открыть URL-ссылку:
  ...
  - `_parent`: в родительском контексте от текущего, то есть уровнем выше. Например, если на страницу вставлен фрейм, а внутри него такая ссылка, то она откроется не внутри фрейма, а в контексте родительской страницы. Если родительского контекста нет, то ссылка откроется в текущем.
  - `_top`: в самом высоком «корневом» контексте от текущего, например, если есть страница, куда вставлен фрейм, в который вставлен фрейм… в общем, ссылка откроется на самом-самом верху.
